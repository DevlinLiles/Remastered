...
void PlayerManagerImplementation::loadLuaConfig() {
	...
	cheapPerformanceBuff = lua->getGlobalInt("cheapPerformanceBuff");
        ...
        //Cost of Custom Non Admin buffs
        expensiveMedicalBuffCost = lua->getGlobalInt("expensiveMedBuffCost");
        expensivePerformanceBuffCost = lua->getGlobalInt("expensivePerfBuffCost");
        cheapBuffCost = lua->getGlobalInt("cheapBuffCost");
...
int PlayerManagerImplementation::awardExperience(CreatureObject* player, const String& xpType,
	...
	if (playerObject == NULL)
		return 0;
	int xp;
	if (xpType == "jedi_general" || amount <= 0 ){
		xp = playerObject->addExperience(xpType, amount);
	} else if (xpType == "imagedesigner" || xpType == "music" || xpType == "dance" || xpType == "entertainer_healing" || xpType == "bio_engineer_dna_harvesting"){
...
void PlayerManagerImplementation::enhanceCharacter(CreatureObject* player, int type) {
	...
	if (type == 3){
                //We are doing the expensive Ent Buffs
		if (player->getCashCredits() < expensivePerformanceBuffCost){
			player->sendSystemMessage("Sorry, you lack the neccessary cash.");
			return;
		} else if (player->getCashCredits() >= expensivePerformanceBuffCost){
			player->subtractCashCredits(expensivePerformanceBuffCost);
			...
			if (message && player->isPlayerCreature())
				player->sendSystemMessage("You have received the highest automated terminal MIND buff.");
			}
	} else if (type == 2){
                //We are doing the expensive Medical Buffs
		if (player->getCashCredits() < expensiveMedicalBuffCost){
			player->sendSystemMessage("Sorry, you lack the neccessary cash.");
			return;
		} else if (player->getCashCredits() >= expensiveMedicalBuffCost){
			player->subtractCashCredits(expensiveMedicalBuffCost);
			...
			if (message && player->isPlayerCreature())
				player->sendSystemMessage("You have received the highest automated terminal HEALTH/ACTION buffs.");
			}
	} else if (type == 1){
                  //We are doing the cheap buffs med and performance
		if (player->getCashCredits() < cheapBuffCost){
			player->sendSystemMessage("Sorry, you lack the neccessary cash.");
			return;
		} else if (player->getCashCredits() >= cheapBuffCost){
			player->subtractCashCredits(cheapBuffCost);
		...
		if (message && player->isPlayerCreature())
			player->sendSystemMessage("An unknown force strengthens you for battles yet to come.");
                ...
                //We are doing the default frog or Admin buff
	...
	if (message && player->isPlayerCreature())
		player->sendSystemMessage("You've been hit with ADMINISTRATIVE juice...");
	}
